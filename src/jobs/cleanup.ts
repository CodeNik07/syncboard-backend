 ...
 *
 * Multi-instance safe: queries MongoDB (shared state) so any instance can
 * perform the cleanup regardless of which instance the user was connected to.
 */
  ...
  setInterval(async () => {
    try {
      ...
      for (const boardId of boardIds) {
        ...
        if (activeCount === 0) {
          // No active users — delete entire board and its Yjs document
